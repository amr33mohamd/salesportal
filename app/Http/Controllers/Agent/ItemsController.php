...
class ItemsController extends Controller
{
    public function index(Request $request){
      $user = Auth::user();
      $leads = $user->items;
        $fields = items::fields()->get();

      return view('Agent.Sales.Items.Items',['leads'=>$leads,'fields'=>$fields]);


    }
    public function editScreen(Request $request){
      $lead = items::query()->where('id',request('id'))->first();
      $user = Auth::user();
...
return view('Agent.Sales.Items.NewItem',['lead'=>$lead,'sources'=>$sources,'mediums'=>$mediums,'industries'=>$industries,'fields'=>$fields,'currancies'=>$currancies,'type'=>'edit','statuses'=>$statuses,'education_qualifications'=>$educations,'hear_about_uses'=>$hear_about_uses,'nationalities'=>$nationalities]);
    ...
    public function addScreen(Request $request){
      ...
      $user = Auth::user();
       ...
       $mediums = traffic_mediums::all();
        ...
        return view('Agent.Sales.Items.NewItem',['lead'=>$lead,'fields'=>$fields,'sources'=>$sources,'mediums'=>$mediums,'industries'=>$industries,'currancies'=>$currancies,'type'=>'add','statuses'=>$statuses,'education_qualifications'=>$educations,'hear_about_uses'=>$hear_about_uses,'nationalities'=>$nationalities]);
    ...
    public function edit(Request $request){
      ...
      //    ::query()->where('id',request('id'))->first()->addMedia($request->file)->toMediaCollection();
      //    unset($data['file']);
      //
      // }
        $account= items::findOrFail(request('id'));
