...
<span>
<span class="vb__breadcrumbs__arrow"></span>
<strong class="vb__breadcrumbs__current">Task</strong>
</span>
</div>
</div>
...
<!-- START: forms/basic-forms-elements -->
          ...
          <!-- Vertical Form -->
          <form method="post" action="@if($type == 'add') {{route('NewTaskAction')}} @else /tasks/edit/action/{{$lead->id}} @endif"  enctype="multipart/form-data">
            @csrf
          ...
          <div class="form-row">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-4">
                <label>Account </label>
                <select class="select2  " name="taskable" data-live-search="true" required>
                  <!-- <option>NA</option> -->
                  @foreach($accounts as $account)
                  <option @if($lead->taskable_type == 'App\Models\accounts' && $lead->taskable_id == $account->id) selected @endif value="account,{{$account->id}}">{{$account->getFieldByName('surname')['value']}}</option>
                  @endforeach
                  @foreach($userLeads as $leadUser)
                  <option @if($lead->taskable_type == 'App\Models\leads' && $lead->taskable_id == $leadUser->id) selected @endif value="lead,{{$leadUser->id}}">{{$leadUser->getFieldByName('surname')['value']}}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-md-4">
