...
<!-- START: forms/basic-forms-elements -->
          ...
          <div class="form-row">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-4">
                <label>Account </label>
                <select class="select2  " name="callable" data-live-search="true" required>
                  <!-- <option>NA</option> -->
                  @foreach($accounts as $account)
                  <option @if($lead->callable_type == 'App\Models\accounts' && $lead->callable_id == $account->id) selected @endif value="account,{{$account->id}}">{{$account->getFieldByName('surname')['value']}}</option>
                  @endforeach
                  @foreach($userLeads as $leadUser)
                  <option @if($lead->callable_type == 'App\Models\leads' && $lead->callable_id == $leadUser->id) selected @endif value="lead,{{$leadUser->id}}">{{$leadUser->getFieldByName('surname')['value']}}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-md-4">
