                                                      ...
                                                      <div class="form-field__control">
                                                          <select id="contact-7" name="callable" class="form-field__input js-example-basic-single">
                                                            ...
                                                            @foreach($accounts as $account)
                                                              ...
                                                              <option value="account,{{$account->id}}" :selected="(editing == 1 &&  {{$account->id}} == current_lead.callable_id) ? true :false">{{$account->getFieldByName('surname')['value'] }}</option>

                                                              @endforeach

                                                              @foreach($userLeads as $accountt)

                                                                <option  value="lead,{{$accountt->id}}" :selected="(editing == 1  && {{$accountt->id}} == current_lead.callable_id) ? true :false">{{$accountt->getFieldByName('surname')['value']}} </option>
